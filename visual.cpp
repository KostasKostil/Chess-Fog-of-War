...
void Visualize(std::string filename)
{
    ...
    while (window->isOpen())
    {
        ...
        if (slides && (clock() - timestamp) > 1.0 * CLOCKS_PER_SEC)
        {
            current_board++;
            timestamp = clock();
        }
...
void VisPlay(std::shared_ptr<Player> p)
{
    ...
    while (window->isOpen())
    {
        ...
        while (window->pollEvent(event))
        {
            ...
            if (wait)
                continue;
            if (event.type == sf::Event::MouseButtonPressed && current_board + 1 == boards.size())
            {
        ...
        if (wait && going && (clock() - timestamp) > 1.0 * CLOCKS_PER_SEC)
        {
            int s = boards.size();
            Move m = p->GetMove(boards[s-2], boards[s-1]);
            wait = 0;
            if (boards.back()[m.to.x][m.to.y].type == Type::King)
                going = false, wait = 1;
